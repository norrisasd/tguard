...
if (isset($_GET['search'])) {
    ...
    $ftype = $ftype == "" ? "" : "AND flagtype.flagtype='$ftype'";
...
if (isset($_FILES['image'])) {
    ...
    if ($error == 0) {
        ...
        $sql = "UPDATE `user` SET `name`='$name',`email`='$email',`username`='$username',`password`='$password',`access`=$access$title WHERE `user_id`=$user_id";
